...
func (w StateMachineHelper) BeforeStep(fn smachine.StateFunc) predicate.Predicate {
	return predicate.And(w.IsSlotLink, predicate.BeforeStep(fn))
}

func (w StateMachineHelper) AfterStep(fn smachine.StateFunc) predicate.Predicate {
	return predicate.And(w.IsSlotLink, predicate.AfterStep(fn))
}

func (w StateMachineHelper) AfterAnyStep() predicate.Predicate {
	return predicate.And(w.IsSlotLink, predicate.AfterAnyStep())
}

func (w StateMachineHelper) AfterAnyMigrate() predicate.Predicate {
	return predicate.And(w.IsSlotLink, predicate.AfterAnyMigrate())
}

func (w StateMachineHelper) AfterMigrate(fn smachine.MigrateFunc) predicate.Predicate {
	return predicate.And(w.IsSlotLink, predicate.AfterMigrate(fn))
}

func (w StateMachineHelper) BeforeStepExt(s smachine.SlotStep) predicate.Predicate {
	return predicate.And(w.IsSlotLink, predicate.BeforeStepExt(s))
}

func (w StateMachineHelper) AfterStepExt(s smachine.SlotStep) predicate.Predicate {
	return predicate.And(w.IsSlotLink, predicate.AfterStepExt(s))
}

func (w StateMachineHelper) AfterTestString(marker string) predicate.Predicate {
	return predicate.And(w.IsSlotLink, predicate.AfterTestString(marker))
}

func (w StateMachineHelper) AfterCustomEvent(fn func(interface{}) bool) predicate.Predicate {
	return predicate.And(w.IsSlotLink, predicate.AfterCustomEvent(fn))
}

func (w StateMachineHelper) AfterCustomEventType(tp reflect.Type) predicate.Predicate {
	return predicate.And(w.IsSlotLink, predicate.AfterCustomEventType(tp))
}

func (w StateMachineHelper) AfterAsyncCall(id smachine.AdapterID) predicate.Predicate {
	return predicate.And(w.IsSlotLink, predicate.AfterAsyncCall(id))
}

func (w StateMachineHelper) AfterResultOfFirstAsyncCall(id smachine.AdapterID) predicate.Predicate {
