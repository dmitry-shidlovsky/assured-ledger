...
func TestVirtual_CallMethodOutgoing_WithPulseChange(t *testing.T) {
	...
	authService := authentication.NewServiceMock(mc)

	authService.HasToSendTokenMock.Return(true) // always send tokens
	// skip token checks, in tests we have only one node, so we need to allow self signed tokens
	authService.IsMessageFromVirtualLegitimateMock.Set(func(_ context.Context, payloadObj interface{}, sender reference.Global, pr pulse.Range) (mustReject bool, err error) {
		return false, nil
	})
	...
	// add ExecutionMocks to runnerMock
	{
		...
		objectBExecutionMock := runnerMock.AddExecutionMock("Foo")
		...
		objectBExecutionMock.AddContinue(
			func(result []byte) {
...

	{
		...
		typedChecker.VDelegatedCallRequest.Set(func(request *payload.VDelegatedCallRequest) bool {
			...
			assert.Equal(t, payload.BuildCallFlags(fooIsolation.Interference, fooIsolation.State), request.CallFlags)
			assert.Equal(t, outgoingCallRef, request.CallOutgoing)
		...
		typedChecker.VDelegatedRequestFinished.Set(func(finished *payload.VDelegatedRequestFinished) bool {
			assert.Equal(t, payload.CTMethod, finished.CallType)
		...
		typedChecker.VCallRequest.Set(func(request *payload.VCallRequest) bool {
			...
			if callCount == 0 {
				assert.Equal(t, payload.CallDelegationToken{}, request.DelegationSpec)
				server.IncrementPulseAndWaitIdle(ctx)
				callCount++
		...
		typedChecker.VCallResult.Set(func(res *payload.VCallResult) bool {
			assert.Equal(t, payload.BuildCallFlags(fooIsolation.Interference, fooIsolation.State), res.CallFlags)

			assert.Equal(t, []byte("finish A.Foo"), res.ReturnArguments)
			assert.Equal(t, payload.CTMethod, res.CallType)
			assert.Equal(t, statemachine.APICaller, res.Caller)
